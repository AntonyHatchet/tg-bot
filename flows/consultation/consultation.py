...
from planfix.planfix import create_task
...
letter_consultation_text = "Письменная консультация в формате «Правового мнения» или «Legal Opinion» - самый предпочтительный вид консультации, " \
    ...
    "Срок составления заключения – 3 рабочих дня. Стоимость – 25 000 рублей. Продолжить оформление этой услуги?"
...
def consultation_type():
    ...
    markup.add(
        ...
        #  (мы пришлем письменное заключение вам на емейл или на почтовый адрес)
...
def personal_prices():
    ...
    markup.row_width = 1
    markup.add(
        InlineKeyboardButton("Юрист из команды Егора Редина 15 000 ₽(2 часа)",
                             callback_data=Callbacks.consultation_personal_price_other.name),
        InlineKeyboardButton("Егор Редин 30 000 ₽(2 часа)",
...
def phone_prices():
    ...
    markup.row_width = 1
    markup.add(
        InlineKeyboardButton("Юрист из команды Егора Редина 5 000 ₽(30 минут)",
                             callback_data=Callbacks.consultation_phone_price_other.name),
        InlineKeyboardButton("Егор Редин 10 000 ₽(30 минут)",
...
# Set category
    if call.data == Callbacks.consultation_litigation.name:
        ...
        msg = bot.send_message(chat_id, user.name +
...
# Type assign
    if call.data == Callbacks.consultation_type_personal.name:
        ...
        msg = bot.send_message(chat_id, price_text,
    ...
    if call.data == Callbacks.consultation_type_letter.name:
        ...
        msg = bot.send_message(
            chat_id, letter_consultation_text, reply_markup=consultation_type())
...
# Set prices
    if call.data == Callbacks.consultation_personal_price_other.name:
        ...
        create_task(name=user.name, phone=user.phone,
